...
def convolve_grayscale(images, kernel, padding='same', stride=(1, 1)):
    ...
    if padding == 'valid':
        ...
        pad_row = int(((img_row - 1) * s_row + kernel_row - img_row) / 2)
        pad_col = int(((img_col - 1) * s_col + kernel_col - img_col) / 2)
    ...
    # i = 0
    # j = 0
    h = int((img_p_col - kernel_col) / s_col) + 1
    w = int((img_p_row - kernel_row) / s_row) + 1
    for j in range(h):
        for i in range(w):
        ...
        #     j += 1
        # i += 1
        # if i < ((img_p_col - kernel_col) / s_col):
        #    i += 1
        # elif j < ((img_p_row - kernel_row) / s_row):
        #    j += 1
        #    i = 0
        # else:
    return result
